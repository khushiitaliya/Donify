...
const seedData = async () => {
  try {
    ...
    const donorPasswords = await Promise.all([
      ...
      bcrypt.hash('donor000', 10),
      bcrypt.hash('donor111', 10)
    ]);
    ...
    const hospitalPasswords = await Promise.all([
      bcrypt.hash('hospital123', 10),
      bcrypt.hash('hospital456', 10),
      bcrypt.hash('hospital789', 10)
    ]);
    ...
    // Calculate urgency scores for each request
    const requestsWithScores = bloodRequests.map(request => {
      const bloodRequest = new BloodRequest(request);
      bloodRequest.calculateUrgencyScore();
      return bloodRequest;
    });

    await BloodRequest.insertMany(requestsWithScores);

    console.log('Database seeded successfully!');
    console.log('\nLogin credentials:');
    console.log('Admin: admin@donify.com / admin123');
    console.log('Donor 1: john@donor.com / donor123');
    console.log('Donor 2: sarah@donor.com / donor456');
